...
public class Homework3 {
  ...
  public static void smoosh(int[] ints) {
    ...
    // 返回void所以不能新建数组只能修改原数组ints，从前往后一个个检查和改值，最后补-1
    int newIndex=0;
    for(int i=1;i<ints.length;i++){//从i=1开始遍历，检查若!=newIndex则newIndex+1值不变为ints[i]，若=则i++
    	if(ints[i]!=ints[newIndex]){
    		ints[++newIndex]=ints[i];//I++是执行完后面的语句才加1；而++I就先做I+1才执行后面的语句。
    	}
    }
    for(int i=newIndex+1;i<ints.length;i++){
    	ints[i]=-1;
    }
  }
