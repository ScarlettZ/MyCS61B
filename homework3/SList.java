...
public class SList {
  ...
  public void squish() {
    ...
    if(head!=null){
	    ...
	    while(node!=null){
	    	if(!newNode.item.equals(node.item)){//main方法中insert了很多Integer进入这个SList，所以item域的dynamic type是Integer，所以这里调用的是Integer的equals方法
	    		newNode.next=node;
	    		size++;
  ...
  public void twin() {
    // Fill in your solution here.  (Ours is seven lines long.)
	    ...
	    while(node!=null){
	    	//newNode=node;//bug：只是新加了一个指针newNode然后让它指向和current同一块内存空间
				newNode=new SListNode(node.item,node.next);//新加了一块内存空间作为新的node
				node.next=newNode;
				node=newNode.next;
	    }
	    size*=2;
