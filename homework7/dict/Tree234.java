...
public class Tree234 extends IntDictionary {
	...
	public void bubbleUp(Tree234Node node){
		if(node==root){
		...
		Tree234Node t1=new Tree234Node(p,node.key1);
		Tree234Node t2=new Tree234Node(p,node.key3);
		if(node.child1!=null)
			node.child1.parent=t1;
		if(node.child2!=null)
			node.child2.parent=t1;
		if(node.child3!=null)
			node.child3.parent=t2;
		if(node.child4!=null)
			node.child4.parent=t2;
		t1.child1=node.child1;
		t1.child2=node.child2;
		t2.child1=node.child3;
		t2.child2=node.child4;
		if(p.keys==1){
			if(node.key2<p.key1){
				p.key2=p.key1;
				p.key1=node.key2;
	...
	public void addKey2Leaf(Tree234Node node,int key){
		if(node.child1!=null)
			return;
		if(key<node.key1){
			node.key3=node.key2;
