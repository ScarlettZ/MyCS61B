  ...
  public Maze(int horizontalSize, int verticalSize) {
     ...
     **/
		...
		while(w>0){
			...
			permutation[index]=permutation[w-1];
			permutation[--w]=temp;			
		}
		int x,y,over,under,left,right;
		w=permutation.length;
		for(i=0;i<w;i++){
			int wall=permutation[i];
			if(wall<horiz*(vert-1)){//horizontal walls
				x=wall%horiz;
				y=wall/horiz;
				over=wall;
				under=wall+horiz;
				int set1=cellSets.find(over);
				int set2=cellSets.find(under);
				if(set1!=set2){
					hWalls[x][y]=false;
					cellSets.union(set1,set2);
				}
			}else{
				x=(wall-(horiz*(vert-1))) % (horiz-1);
        y=(wall-(horiz*(vert-1))) / (horiz-1);
        left=wall-(horiz*(vert-1));
        right=wall-(horiz*(vert-1))+1;
        int set1=cellSets.find(left);
				int set2=cellSets.find(right);
				if(set1!=set2){
					vWalls[x][y]=false;
					cellSets.union(set1,set2);
				}
