...
class SibTreeNode extends TreeNode {
	...
	public void insertChild(Object item, int c) throws InvalidNodeException {
		...
		if(c<=1){
			...
			firstChild=newChild;
		}else if(c>1&&children()<c){
			c=children()+1;
			newChild.nextSibling=null;
			((SibTreeNode)child(c-1)).nextSibling=newChild;
		}else{
			newChild.nextSibling=(SibTreeNode)child(c);
			((SibTreeNode)child(c-1)).nextSibling=newChild;
		}
		myTree.size++;
	}
	...
	public void removeLeaf() throws InvalidNodeException {
		...
		if(this==node){
			...
			while(node.nextSibling().isValidNode()){
				if(node.nextSibling()==this){
					node.nextSibling=(SibTreeNode) this.nextSibling;
					break;
				}
				node=(SibTreeNode) node.nextSibling();
			}
		}
		this.valid=false;
		myTree.size--;
