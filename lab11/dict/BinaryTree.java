...
public class BinaryTree implements Dictionary {
  ...
  public Entry remove(Object key) {
    // Replace the following line with your solution.
    BinaryTreeNode node = findHelper((Comparable) key, root);
    //int comp=((Comparable)key).compareTo(node.parent.entry.key());
    if(node==null)
    	return null;
    else if(node.leftChild==null&&node.rightChild==null){
    	if(node!=root)
	    	if(node.parent.leftChild == node)//node是左节点
	    		node.parent.leftChild=null;
	    	else
	    		node.parent.rightChild=null;
    	else
    		root=null;
    }else if(node.leftChild!=null&&node.rightChild==null){//node有且只有左孩子
    	if(node!=root)
	    	if(node.parent.leftChild == node)//node是左节点
    ...
    }else if(node.leftChild==null&&node.rightChild!=null){//node有且只有右孩子
    	if(node!=root)
	    	if(node.parent.leftChild == node)//node是左节点
